...
<div id="title">Archive 2015 January - June</div>
<br>

<?php $number = count($posts);
 foreach ($posts as $post) {
    if ($post['front_page_id'] <= $number - 15 && (strpos($post['date'], '2015')) && (strpos($post['date'], 'January') || strpos($post['date'], 'February ') || strpos($post['date'], 'March') || strpos($post['date'], 'April') || strpos($post['date'], 'May ') || strpos($post['date'], 'May'))) { ?>
