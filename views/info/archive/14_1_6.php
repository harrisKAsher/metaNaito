...
<div id="title">Archive 2014 January - June</div>
<br>

<?php $number = count($posts);
 foreach ($posts as $post) {
    if ($post['front_page_id'] <= $number - 15 && (strpos($post['date'], '2014')) && (strpos($post['date'], 'January') || strpos($post['date'], 'February ') || strpos($post['date'], 'March') || strpos($post['date'], 'April') || strpos($post['date'], 'May ') || strpos($post['date'], 'May'))) { ?>
...
<a href="/index.php?action=IARC14_7_12" class="ToTheLeft">Newer News</a>
<a href="/index.php?action=IARC13_7_12" class="ToTheRight">Older News</a>
