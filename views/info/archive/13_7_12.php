...
<div id="title">Archive 2013 July - December</div>
<br>

<?php $number = count($posts);
 foreach ($posts as $post) {
    if ($post['front_page_id'] <= $number - 15 && (strpos($post['date'], '2013')) && (strpos($post['date'], 'July') || strpos($post['date'], 'August ') || strpos($post['date'], 'September') || strpos($post['date'], 'October') || strpos($post['date'], 'November ') || strpos($post['date'], 'December'))) { ?>
...
<a href="/index.php?action=IARC14_1_6" class="ToTheLeft">Newer News</a>
<a href="/index.php?action=IARC13_1_6" class="ToTheRight">Older News</a>
