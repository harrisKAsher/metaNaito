...
<div id="title">Archive 2015 July - December</div>
<br>

<?php $number = count($posts);
 foreach ($posts as $post) {
    if ($post['front_page_id'] <= $number - 15 && (strpos($post['date'], '2015')) && (strpos($post['date'], 'July') || strpos($post['date'], 'August ') || strpos($post['date'], 'September') || strpos($post['date'], 'October') || strpos($post['date'], 'November ') || strpos($post['date'], 'December'))) { ?>
